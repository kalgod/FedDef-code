...
class APGDT():
    ...
    def __init__(self, model,net2, norm='Linf', eps=8/255, steps=10, n_restarts=1, seed=0, eot_iter=1, rho=.75, verbose=False, n_classes=10):
        super(APGDT, self).__init__()
        self.eps = eps
        self.steps = steps
        self.norm = norm
    ...
    def dlr_loss_targeted(self, x, y, y_target):
        ...
        if x_sorted.shape[1]>2: return -(x[np.arange(x.shape[0]), y] - x[np.arange(x.shape[0]), y_target]) / (
            x_sorted[:, -1] - .5 * x_sorted[:, -3] - .5 * x_sorted[:, -4] + 1e-12)
        else: return -(x[np.arange(x.shape[0]), y] - x[np.arange(x.shape[0]), y_target]) / (
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                loss = loss_indiv.sum()
                loss = -torch.mean(self.model.subexecute(x_adv))
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        self.target_class = 0
        best_curr, acc, loss_curr, adv = self.attack_single_run(x, y)  # nopep8
        return acc, adv
